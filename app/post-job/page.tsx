import Link from "next/link";

const requirements = ["Responsive landing page", "Deployed preview", "Source link"];

export default function PostJobPage() {
  return (
    <main className="flex flex-1 flex-col bg-[var(--background)] px-4 py-10 font-mono text-[var(--text-primary)] sm:px-6 lg:px-8">
      <section className="mx-auto grid w-full max-w-[980px] gap-5 lg:grid-cols-[1fr_320px]">
        <div className="rounded-[18px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow-soft)] sm:p-8">
          <p className="text-[13px] font-black uppercase text-[var(--text-muted)]">
            2. Create job
          </p>
          <h1 className="mt-3 text-[42px] font-black leading-none sm:text-[62px]">
            Lock
            <br />
            escrow
          </h1>

          <div className="mt-8 grid gap-4">
            <label className="grid gap-2">
              <span className="text-[13px] font-black uppercase text-[var(--text-muted)]">
                Title
              </span>
              <input
                className="h-12 rounded-[10px] border border-[var(--border-strong)] bg-[var(--surface-elevated)] px-4 text-[15px] font-black outline-none"
                defaultValue="Landing page design"
              />
            </label>
            <label className="grid gap-2">
              <span className="text-[13px] font-black uppercase text-[var(--text-muted)]">
                Budget
              </span>
              <input
                className="h-12 rounded-[10px] border border-[var(--border-strong)] bg-[var(--surface-elevated)] px-4 text-[15px] font-black outline-none"
                defaultValue="0.42 ETH"
              />
            </label>
            <label className="grid gap-2">
              <span className="text-[13px] font-black uppercase text-[var(--text-muted)]">
                Requirements
              </span>
              <textarea
                className="min-h-32 rounded-[10px] border border-[var(--border-strong)] bg-[var(--surface-elevated)] px-4 py-3 text-[15px] leading-7 outline-none"
                defaultValue={requirements.join("\n")}
              />
            </label>
          </div>

          <Link
            className="mt-6 inline-flex h-[50px] w-full items-center justify-center rounded-[10px] bg-[var(--button)] px-6 text-[15px] font-black text-[var(--button-text)] transition hover:bg-[var(--accent-hover)] sm:w-auto"
            href="/jobs/landing-page-implementation"
          >
            Create + fund
          </Link>
        </div>

        <aside className="rounded-[18px] border border-[var(--border)] bg-[var(--surface)] p-5">
          <p className="text-[13px] font-black uppercase text-[var(--text-muted)]">Escrow</p>
          <p className="mt-4 text-[38px] font-black leading-none">0.42 ETH</p>
          <p className="mt-3 text-[14px] leading-6 text-[var(--text-secondary)]">
            Locked until proof is approved.
          </p>
        </aside>
      </section>
    </main>
  );
}
